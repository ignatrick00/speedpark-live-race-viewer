...
export default function RaceBrowser() {
  ...
  const fetchRaces = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/races?date=${selectedDate}`);
      const data = await response.json();

      if (data.success) {
        setRaces(data.races);
        console.log(`📊 Loaded ${data.races.length} races for ${selectedDate}`);
      }
    } catch (error) {
  ...
  const fetchRaceResults = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/race-results?sessionId=${encodeURIComponent(selectedRace.sessionId)}`);
      const data = await response.json();

      if (data.success) {
        setRaceResults(data.results);
        console.log(`🏁 Loaded results for ${selectedRace.sessionName}`);
      }
    } catch (error) {
              ...
              {races.map((race) => (
                ...
                >
                  ...
                  <div className="text-sm text-sky-blue/60 space-y-1">
                    ...
                    <div>👥 {race.driverCount} pilotos</div>
        ...
        <div className="bg-gradient-to-br from-racing-black/90 to-racing-black/70 border border-electric-blue/20 rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-sky-blue/60">
                {selectedRace.displayDate} • {selectedRace.displayTime} • {selectedRace.driverCount} pilotos
              </p>
            </div>
            <button
              onClick={() => {
