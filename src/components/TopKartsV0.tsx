...
export default function TopKartsV0({ period, title, subtitle }: Props) {
  const [bestTimes, setBestTimes] = useState<KartBestTime[]>([]);
  ...
  const fetchBestTimes = async () => {
    try {
      ...
      const response = await fetch(`/api/best-times-v0?period=${period}&type=karts`);
      ...
      console.error('Error fetching kart best times:', error);
    } finally {
      if (isFirstLoad) {
        setLoading(false);
