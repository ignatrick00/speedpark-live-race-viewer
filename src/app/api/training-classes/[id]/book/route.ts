...
export async function POST(
  ...
  try {
    ...
    const { bookingType } = body; // 'individual' or 'group'
    ...
    // Find training class
    const trainingClass = await TrainingClass.findById(params.id);
    if (!trainingClass) {
      return NextResponse.json(
    ...
    if (trainingClass.status === 'cancelled') {
      return NextResponse.json(
    ...
    if (bookingType === 'individual') {
      // Check availability - only consider it occupied if there's a booking with studentId
    ...
    if (bookingType === 'group') {
      // Check availability - only block if there's an actual individual booking with studentId
    ...
    console.error('Error booking training class:', error);
    return NextResponse.json(
