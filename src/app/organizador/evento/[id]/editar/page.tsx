...
export default function EditarEventoPage() {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        const response = await fetch(`/api/squadron-events/${eventId}`, {
          headers: {
        ...
        // Combinar eventDate y eventTime en un solo datetime string
        const eventDateTime = new Date(`${event.eventDate.split('T')[0]}T${event.eventTime}`);
        const registrationDateTime = new Date(event.registrationDeadline);

        setFormData({
          name: event.name || '',
          description: event.description || '',
          category: event.category || EventCategory.OPEN_SERIES,
          eventDate: eventDateTime.toISOString().slice(0, 16), // YYYY-MM-DDTHH:MM
          duration: event.duration || 90,
          registrationDeadline: registrationDateTime.toISOString().slice(0, 16),
          location: event.location || 'SpeedPark',
          maxSquadrons: event.maxSquadrons || 20,
          minPilotsPerSquadron: event.minPilotsPerSquadron || 2,
          maxPilotsPerSquadron: event.maxPilotsPerSquadron || 6,
        });
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/organizador/evento/${eventId}`);
        router.refresh();
      } else {
      ...
      setError('Error de red al actualizar evento');
    } finally {
      setLoading(false);
    }
  };
    ...
    <OrganizerGuard>
      <div className="min-h-screen bg-black text-white relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto p-8">
          ...
          <div className="mb-8">
            <button
              onClick={() => router.push(`/organizador/evento/${eventId}`)}
            ...
            <p className="text-gray-400">Modifica la configuración del evento</p>
          </div>
