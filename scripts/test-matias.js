...
async function test() {
  ...
  console.log('🔍 Buscando usuarios con "Matias"...\n');
  
  const users = await WebUser.find({
    $or: [
      { 'profile.alias': /Matias/i },
      { 'kartingLink.speedParkProfile.driverName': /Matias/i },
      { 'profile.firstName': /Matias/i }
    ]
  }).lean();
  ...
  // Buscar escudería Mclaren
  console.log('\n🏁 Buscando escudería Mclaren...\n');
  const mclarenSquadron = await Squadron.findOne({ name: /mclaren/i, isActive: true }).lean();
  if (mclarenSquadron) {
    console.log('   Nombre:', mclarenSquadron.name);
    console.log('   Members:', mclarenSquadron.members);
    console.log('   CaptainId:', mclarenSquadron.captainId);
  }
